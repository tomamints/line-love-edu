...
相談者のトーク履歴のパターンから心理状態を推測し、その気持ちに寄り添うアドバイスを提供してください。

重要な指針：
1. 実際の会話内容を直接引用せず、会話パターンから相談者の心理を推測する
2. 「深夜のメッセージから感じる不安」「返信を待つ時の気持ち」など、相談者の感情に共感する
3. 相談者の悩み（${wantToKnow}）に対して、心の持ち方や見方を変える視点を提供する
4. 具体的な行動指示ではなく、心の在り方を月の満ち欠けに例えて伝える
5. 会話の特徴（質問の有無: ${hasQuestions}, 絵文字使用: ${hasEmojis}, メッセージ長: ${hasLongMessages ? '長い' : '短い'}）から心理状態を読み取る
...
{
  "introductionMessage": "序章のパーソナライズされた導入文（400文字以上。${user.name}様への呼びかけから始まり、恋愛状況'${loveSituation === 'beginning' ? '恋の始まり' : loveSituation === 'relationship' ? '交際中' : loveSituation === 'complicated' ? '複雑な事情' : '終わり・復縁'}'と知りたいこと'${wantToKnow === 'feelings' ? '相手の気持ち' : wantToKnow === 'action' ? 'どう行動すべきか' : wantToKnow === 'past' ? '過去の意味' : 'これからどうなるか'}'に深く共感する月詠からのメッセージ。月の満ち欠けに例えながら、これから始まる占いへの期待を高める内容）",
  "personalizedLetter": "相談者への個別化された手紙（1000文字以上。月詠として、月の満ち欠けや月相の表現を多用した詩的な文章で。重要：実際の会話内容を直接引用せず、会話パターンから読み取れる相談者の心理状態を推測して寄り添う。例えば『深夜のメッセージが多いことから、相手のことを考えて眠れない夜があるのでしょう』『返信を待つ間の不安な気持ち』『既読がついた瞬間の安堵感』など、相談者が感じているであろう感情に共感する。相談者の悩み'${wantToKnow === 'feelings' ? '相手の気持ち' : wantToKnow === 'action' ? 'どう行動すべきか' : wantToKnow === 'past' ? '過去の意味' : 'これからどうなるか'}'に対して、月の導きとして明確な答えを提供。具体的な行動の提案ではなく、心の持ち方や見方を変える視点を与える。数値やパーセントは使わず、月の表現を使って詩的に表現）",
  "coreAnswer": ${(() => {
    if (wantToKnow === 'feelings') {
      return `"相手の本当の気持ちについての核心的な答え（500文字以上。会話パターン、返信時間、絵文字の使い方、会話の盛り上がり方などから推測される相手の心理状態を詳細に分析。『相手はあなたに好意を持っている兆候が...』『まだ迷いがあるようですが...』など、相談者が最も知りたい相手の気持ちについて、月詠として優しく、しかし明確に伝える）"`;
    } else if (wantToKnow === 'action') {
      return `"今すぐ取るべき行動についての具体的な指針（500文字以上。現在の関係性の段階、会話パターンから見える最適なアプローチ方法を提案。『今は待つ時期』『一歩踏み出す勇気を』『自然体でいることが大切』など、状況に応じた明確な行動指針を月の満ち欠けに例えながら提供）"`;
    } else if (wantToKnow === 'past') {
      return `"過去の出来事が持つ意味の解釈（500文字以上。会話パターンの変化、特定の時期の盛り上がりや沈黙が意味していたこと、なぜそのような出来事が起きたのかを心理学的に分析。『あの時の沈黙は、実は相手が真剣に考えていた証』『急に会話が減ったのは...』など、過去を振り返って納得できる説明を提供）"`;
    } else {
      return `"これからの関係性の行方についての予測（500文字以上。現在の会話パターン、関係性の成熟度、二人の相性から予測される3ヶ月後、半年後の未来像。『このまま順調に進めば...』『転機が訪れる可能性が...』など、希望を持ちながらも現実的な未来予測を月の周期になぞらえて説明）"`;
    }
  })()},
  "empathyMessage": "相談者への共感メッセージ（150文字。「〜ですよね」「お気持ちよく分かります」など共感を示す）",
  "tsukuyomiComments": {
    "weeklyPattern": "曜日別パターンの分析（200文字。会話の頻度や雰囲気から読み取れる曜日ごとの心理状態を分析。月の満ち欠けに例えて表現）",
    "hourlyPattern": "時間帯パターンの分析（200文字。その時間帯に会話が多い理由を心理的側面から分析。朝の残月、夕月、月光などの表現を使用）",
    "conversationQuality": "会話の質の分析（200文字。会話のリズムや間から読み取れる二人の心の距離感を月の引力になぞらえて表現）",
    "overallDiagnosis": "総合診断（200文字。会話パターンから感じ取れる二人の関係性を月の満ち欠けのサイクルに例えて説明）",
    "fivePillars": "5つの柱の分析（200文字。強みを月の輝きに、改善点を月の影に例えて、優しく伝える）",
  ...
  "suggestedActions": [
    {
      "title": "今すぐできること",
      "action": "会話パターンから導き出される心の持ち方の提案（150文字。返信を待つ時の心構え、相手の沈黙の受け止め方など）",
      "timing": "今すぐ",
      "successRate": 85,
      "reason": "なぜこの心の持ち方が大切か（100文字）",
      "moonGuidance": "月からの導き（50文字。月の満ち欠けに例えた励まし）"
    },
    {
      "title": "1週間後の心構え",
      "action": "会話パターンの変化を受け入れる心の準備（150文字。関係性の自然な変化を月の満ち欠けに例えて）",
      "timing": "1週間後",
      "successRate": 90,
      "reason": "なぜこの時期に心の準備が必要か（100文字）",
      "moonGuidance": "月からの導き（50文字。上弦の月から満月への変化に例えて）"
    },
    {
      "title": "長期的な心の成長",
      "action": "1ヶ月かけて育む心の在り方（150文字。相手を信じる力、待つ力の育み方）",
      "timing": "1ヶ月後",
      "successRate": 95,
      "reason": "時間をかけて心を育てる意味（100文字）",
      "moonGuidance": "月からの導き（50文字。月の満ち欠けの完全なサイクルに例えて）"
    }
  ],
  ...
  "closingMessage": "終章のパーソナライズされた締めくくり（500文字以上。${user.name}様への最後のメッセージ。総合スコア${fortune.totalScore || 0}点と恋愛状況を踏まえ、今後の関係性への励ましと祝福。月の満ち欠けの美しさに例えながら、変化を恐れず前に進む勇気を与える内容。最後は『月詠より、愛と祝福を込めて』で締める）"
}`;
  }
