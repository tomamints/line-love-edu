...
[
  {
    ...
    type: "text"   // text, image, sticker, etc.
  },
  {
    id: "msg_002",
    text: "お疲れ様〜！今日は何してたの？",
...
| 項目 | おつきさま診断（無料） | プレミアムレポート（有料） |
|------|---------------------|------------------------|
| 分析メッセージ数 | 最新30件程度 | **全メッセージ** |
| AI利用 | なし | GPT-4o（16,384トークン） |
| 分析深度 | 表面的な統計のみ | 心理分析・感情分析含む |
| アウトプット | LINE FlexMessage | PDF（12-15ページ） |
| 処理時間 | 即時（1-2秒） | 1-2分（Batch API利用） |
| アクションプラン | 簡単なアドバイス1文 | 具体的な行動計画 |
| 会話テンプレート | なし | 状況別の例文提供 |
| 将来予測 | なし | 成功確率と時期予測 |