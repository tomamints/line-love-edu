#!/usr/bin/env python3
"""
スーパームーン、暁月、十三夜月の枠をトリミング
"""

from PIL import Image
import os

def trim_borders(image_path, trim_pixels):
    """
...
def main():
    ...
    target_files = {
        "super-moon.jpg": 45,          # スーパームーン - 黒い枠を削除
        "waning-crescent.jpg": 50,     # 暁月 - ベージュと黒の二重枠を削除
        "waxing-gibbous.jpg": 40,      # 十三夜月 - 黒い枠を削除
    }
    ...
    for filename, trim_pixels in target_files.items():
        ...
        if os.path.exists(file_path):
            if trim_borders(file_path, trim_pixels):
                processed_count += 1
        else:
